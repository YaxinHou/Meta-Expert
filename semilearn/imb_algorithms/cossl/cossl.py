...
class CoSSL(ImbAlgorithmBase):
    ...
    def set_data_loader(self):
        ...
        if self.tfe_augment == 'weak':
            tfe_labeled_set.transform = self.dataset_dict['train_ulb'].weak_transform
        ...
        if self.args.noise_ratio > 0:
            sampler_x = get_weighted_sampler(class_balanced_disb, torch.Tensor(self.lb_cnt_per_class),
                                         tfe_labeled_set.noised_targets)
        else:
            sampler_x = get_weighted_sampler(class_balanced_disb, torch.Tensor(self.lb_cnt_per_class),
    ...
    def train_step(self, x_lb, y_lb, y_lb_noised, x_ulb_w, x_ulb_s, y_ulb):
        ...
        try:
            ...
            if self.args.noise_ratio > 0:
                tfe_targets_x = labeled_dict['y_lb_noised']
            else:
        ...
        with torch.no_grad():
            ...
            new_feat_tensor = torch.stack(new_feat_list, dim=0)
            new_target_tensor = torch.stack(new_target_list, dim=0)
    ...
    def get_argument():
        return [
            ...
            SSL_Argument('--cossl_warm_epoch', int, 0),
            SSL_Argument('--cossl_tfe_warm_epoch', int, 1),
