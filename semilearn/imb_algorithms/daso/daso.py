...
class DASO(ImbAlgorithmBase):
    ...
    def imb_init(self, T_proto=0.05, T_dist=1.5, daso_queue_len=256, interp_alpha=0.3, with_dist_aware=True, assign_loss_ratio=1.0, num_pl_dist_iter=100, num_pretrain_iter=5120):
    ...
    def train_step(self, x_lb, y_lb, y_lb_noised, x_ulb_w, x_ulb_s, y_ulb):
        ...
        with torch.no_grad():
            ...
            self.queue.enqueue(feats_x_lb.clone().detach(), lb.clone().detach())
        self.ema.restore()
